...
function Dashboard() {
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      {/* 错误提示 */}
      {error && (
        <Alert message="⚠️ 加载失败" description={error} type="error" showIcon style={{ marginBottom: 16 }} />
      )}
              ...
              <List dataSource={data?.activities || []} locale={{ emptyText: '暂无活动' }} renderItem={(item) => (
                <List.Item>
                  <List.Item.Meta
                    title={
                      <Space>
                        <Tag color={item.status === 'completed' ? 'green' : item.status === 'in_progress' ? 'blue' : item.status === 'failed' ? 'red' : 'default'}>{item.action}</Tag>
