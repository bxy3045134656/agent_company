        ...
        <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
              ...
              <div style={{ marginTop: 16 }}>
                <Space direction="vertical" size="small" style={{ width: '100%' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Badge status="success" text={data?.agents?.online || 0} />
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Text type="secondary">忙碌</Text>
                    <Badge status="processing" text={data?.agents?.busy || 0} />
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Text type="secondary">离线</Text>
                    <Badge status="default" text={data?.agents?.offline || 0} />
                  </div>
                </Space>
              </div>
            </Card>
              ...
              <div style={{ marginTop: 16 }}>
                <Space direction="vertical" size="small" style={{ width: '100%' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Badge status="processing" text={data?.tasks?.inProgress || 0} />
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Text type="secondary">已完成</Text>
                    <Badge status="success" text={data?.tasks?.completed || 0} />
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Text type="secondary">待处理</Text>
                    <Badge status="default" text={data?.tasks?.pending || 0} />
                  </div>
                </Space>
              </div>
            </Card>
          ...
          <Col xs={24} sm={12} lg={6}>
            <Card hoverable>
              <Statistic
                title="今日完成"
                value={data?.tasks?.todayCompleted || 0}
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <Progress 
                  percent={Math.round((data?.tasks?.todayCompleted || 0) / Math.max((data?.tasks?.total || 1), 1) * 100)} 
                  size="small"
                  strokeColor={{
                    '0%': '#faad14',
                    '100%': '#52c41a',
          ...
          <Col xs={24} sm={12} lg={6}>
            <Card hoverable>
              <Statistic
                title="项目进度"
                value={data?.projects?.[0]?.progress || 0}
                suffix="%"
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <Progress 
                  percent={data?.projects?.[0]?.progress || 0} 
                  strokeColor={{
                    '0%': '#722ed1',
                    '100%': '#13c2c2',
                  }}
                ...
                <div style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between' }}>
                  <Text type="secondary" size="small">
                    已完成 {data?.projects?.[0]?.completedTasks || 0}/{data?.projects?.[0]?.totalTasks || 0}
                  </Text>
                </div>
              </div>
            </Card>
                ...
                renderItem={(item) => (
                  <List.Item>
                    <List.Item.Meta
                      ...
                      description={
                        <Space>
                          <Text type="secondary">负责人：{item.agent}</Text>
